...
import type { ManagerHistoryPage } from "@/actions/super-admin/managers/types";

import { prisma } from "@/lib/prisma";
import { getSession } from "@/lib/session";

interface GetManagerHistoryOptions {
  cursor?: number | null;
  limit?: number;
...
function compactPayload(log: { action: "CREATE" | "UPDATE" | "DELETE"; oldValue: string | null; newValue: string | null }) {
...
function buildCreatedAtFilter(options: GetManagerHistoryOptions) {
...
export async function getManagerHistory(managerId: number, options: GetManagerHistoryOptions = {}): Promise<ManagerHistoryPage> {
