...
import type { SupplierHistoryPage, SupplierHistoryRow } from "../types/supplier";

interface GetSupplierHistoryOptions {
  cursor?: number | null;
  limit?: number;
...
function compactPayload(log: SupplierHistoryRow) {
...
function buildCreatedAtFilter(options: GetSupplierHistoryOptions) {
...
export async function getSupplierHistory(id: number, options: GetSupplierHistoryOptions = {}): Promise<SupplierHistoryPage> {
  ...
  return {
    rows: pageRows.map((row) => {
      const mapped = {
        id: row.id,
        action: row.action as SupplierHistoryRow["action"],
        entity: row.entity,
