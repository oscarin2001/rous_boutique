...
const ACTION_LABELS: Record<SupplierHistoryRow["action"], string> = {
  CREATE: "Creacion",
...
function formatValue(key: string, value: unknown): string {
  ...
  if ((key === "birthDate" || key === "partnerSince" || key === "contractEndAt") && typeof value === "string") {
  ...
  if (Array.isArray(value)) {
    if (key === "branchIds") return value.length > 0 ? `${value.length} sucursal(es)` : "Sin sucursal asignada";
    if (key === "managerIds") return value.length > 0 ? `${value.length} encargado(s)` : "Sin encargado asignado";
...
function keysForRow(row: SupplierHistoryRow, oldPayload: AuditPayload | null, newPayload: AuditPayload | null): string[] {
  const keys = Array.from(new Set([...payloadKeys(oldPayload), ...payloadKeys(newPayload)]));
  if (row.action !== "UPDATE" && row.action !== "STATUS_CHANGE") return keys;
  return keys.filter((key) => !areEqual(oldPayload?.[key], newPayload?.[key]));
}

function describeChange(action: SupplierHistoryRow["action"], key: string, oldValue: unknown, newValue: unknown): string {
...
function actionContextLabel(action: SupplierHistoryRow["action"]): string {
  if (action === "CREATE") return "creacion";
  if (action === "UPDATE" || action === "STATUS_CHANGE") return "actualizacion";
  return "eliminacion";
}
            ...
            {rows.map((row) => (
              <article key={row.id} className="rounded-lg border bg-muted/20 p-3">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                  ...
                  <span className="text-xs font-medium text-foreground">Responsable: {row.employeeName ?? "Sistema"}</span>
                </div>
