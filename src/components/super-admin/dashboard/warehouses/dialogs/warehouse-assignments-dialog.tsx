...
interface Props {
  ...
  branchOptions: WarehouseOptionBranch[];
  managerOptions: WarehouseOptionManager[];
  isPending: boolean;
  onSave: (data: Record<string, unknown>, id: number) => Promise<WarehouseActionResult>;
}

export function WarehouseAssignmentsDialog({ open, onOpenChange, row, branchOptions, managerOptions, isPending, onSave }: Props) {
  ...
  useEffect(() => {
    if (!open || !row) return;
    setBranchIds(row.branches.map((item) => item.id));
    setManagerIds(row.managers.map((item) => item.id));
    setConfirmName("");
    setConfirmPassword("");
    setMessage(null);
  }, [open, row]);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      address: row.address,
      city: row.city,
      department: row.department && BOLIVIA_DEPARTMENTS.includes(row.department as (typeof BOLIVIA_DEPARTMENTS)[number])
        ? row.department
        : BOLIVIA_DEPARTMENTS[0],
      country: BOLIVIA_COUNTRY,
      openedAt: row.openedAt ? row.openedAt.slice(0, 10) : "",
      branchIds,
      managerIds,
      confirmPassword,
    };

    const result = await onSave(payload, row.id);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sa-modal-wide">
        <DialogHeader>
          ...
          <DialogDescription>Configura sucursales y encargados de la bodega sin editar datos base.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2">
          <Label>Escribe {row?.name} para confirmar</Label>
